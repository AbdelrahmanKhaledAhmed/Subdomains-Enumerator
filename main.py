...
def send_notification(message):
    ...
    if ntfy_topic:
        try:
            ...
            console.print(Panel("[red]FAILED TO SEND NTFY NOTIFICATION[/red]", border_style="red"))
    ...
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, 'r') as f:
                ...
                if ntfy_topic:
                    console.print(Panel(f"[green]LOADED NTFY TOPIC FROM CONFIG: [cyan]{ntfy_topic}[/cyan][/green]", border_style="green"))
                    return
        except Exception:
            console.print(Panel("[red]ERROR READING CONFIG FILE. ASKING MANUALLY...[/red]", border_style="red"))

    ntfy_topic = Prompt.ask("[bold cyan]ENTER NTFY.SH TOPIC NAME (OR PRESS ENTER TO SKIP)[/bold cyan]").strip()
    
    if ntfy_topic:
        try:
            with open(CONFIG_FILE, 'w') as f:
                json.dump({"ntfy_topic": ntfy_topic}, f)
            console.print(Panel(f"[green]TOPIC SAVED TO [bold]config.json[/bold] FOR FUTURE USE.[/green]", border_style="green"))
...
def main():
    ...
    website_name = Prompt.ask("[bold cyan] PLEASE ENTER THE NAME OF THE FOLDER [/bold cyan]").strip()

    BASE_OUTPUT_DIR = r"W:\BugBounty"
    path = os.path.join(BASE_OUTPUT_DIR, website_name)
    os.makedirs(path, exist_ok=True)

    console.print(Panel("[bold yellow]ENTER DOMAIN(S). SUBMIT EMPTY LINE TO FINISH.[/bold yellow]", border_style="yellow"))
    domains = []
    while True:
    ...
    crtsh_script = os.path.join(BASE_DIR, "crtsh.py")
    ...
    console.print(Panel(f"[bold green]SUBFINDER FOUND [cyan]{len(subfinder_lines)}[/cyan] UNIQUE SUBDOMAINS.[/bold green]", border_style="green"))
    ...
    console.print(Panel(f"[bold green]DEEP SUBFINDER FOUND [cyan]{len(deep_lines)}[/cyan] UNIQUE SUBDOMAINS.[/bold green]", border_style="green"))
    ...
    console.print(Panel(f"[bold green]ASSETFINDER FOUND [cyan]{len(assetfinder_lines)}[/cyan] UNIQUE SUBDOMAINS.[/bold green]", border_style="green"))
    send_notification(f"Assetfinder Finished. Found {len(assetfinder_lines)} Subdomains.")

    # crt.sh
    crtsh_lines = run_command_with_spinner(
        f"python \"{crtsh_script}\" -d { ' '.join(domains) } > \"{crtsh_file}\"",
        "Running crt.sh",
        crtsh_file
    )
    console.print(Panel(f"[bold green]CRT.SH FOUND [cyan]{len(crtsh_lines)}[/cyan] UNIQUE SUBDOMAINS.[/bold green]", border_style="green"))
    ...
    console.print(Panel(f"[bold green]TOTAL UNIQUE SUBDOMAINS: [cyan]{len(combined_domains)}[/cyan][/bold green]", border_style="green"))
    ...
    console.print(Panel(f"[bold green]HTTPX 2XX FOUND [cyan]{len(httpx_2xx_lines)}[/cyan] SUBDOMAINS.[/bold green]", border_style="green"))
    ...
    console.print(Panel(f"[bold green]HTTPX 3XX FOUND [cyan]{len(httpx_3xx_lines)}[/cyan] SUBDOMAINS.[/bold green]", border_style="green"))
    ...
    console.print(Panel(f"[bold green]HTTPX 4XX FOUND [cyan]{len(httpx_4xx_lines)}[/cyan] SUBDOMAINS.[/bold green]", border_style="green"))
